  ...
  GType type)
{
  ...
  namespace_context = g_newa (GdartBridgeContextNamespaceGtype, 1);
  namespace_context->namespace = namespace_;
    ...
    GError **error)
{
  ...
  if (!base_info_klass->retrieve_wrapping_class (base_info,
      self,
      namespace_,
      &dart_type,
      dart_error_out,
...
static void _gi_base_info_unref_generic (gpointer generic)
{
  g_base_info_unref ( (GIBaseInfo *) generic);
}
static gpointer _gi_base_info_ref_generic (gpointer generic)
{
  return (gpointer) g_base_info_ref ( (GIBaseInfo *) generic);
}
    ...
    GError **error)
{
  ...
  GIBaseInfo *base_info;
  GITypelib *gi_repo_typelib;

  gi_repo_typelib = g_irepository_require (gdart_bridge_context_get_gi_repository (self),
                    "GIRepository",
                    NULL,
                    0,
                    error);
  if (gi_repo_typelib == NULL) {
    *dart_error_out = gdart_bridge_context_create_error_handle (self,
                      "%s: Error from g_irepository_require: %s",
                      G_STRFUNC,
                      (*error)->message
                                                               );
    return NULL;
  }
  base_info = g_irepository_find_by_name (gdart_bridge_context_get_gi_repository (self),
                                          "GIRepository",
                                          "BaseInfo");
  if (base_info == NULL) {
    *dart_error_out = gdart_bridge_context_create_error_handle (self,
                      "%s: Could not find a required class for static GObject info.",
                      G_STRFUNC);
  ...
  object_wrapper->object = object_info;
  object_wrapper->object_info = base_info;
  ...
  object_wrapper->copy_func = _gi_base_info_ref_generic;
  object_wrapper->free_func = _gi_base_info_unref_generic;

  internal_container = gdart_bridge_context_wrap_pointer (
                         self, object_wrapper, gdart_bridge_context_finalize_wrapped_custom_struct,
                         dart_error_out, error);

  if (internal_container == NULL) {
    return NULL;
  }
  wrapped_object = gdart_bridge_context_wrap_internal_pointer (
                     self, internal_container, "GIRepository",
                     g_registered_type_info_get_g_type (base_info),
                     base_info,
                     &gi_registered_type_info_registered_type_info,
                     dart_error_out,
                     error
...
GIRegisteredTypeInfo *gdart_bridge_context_unwrap_object_info (GdartBridgeContext *self,
    ...
    GError **error)
{
  GdartBridgeContextWrappedObject *object;
  object = gdart_bridge_context_unwrap_object (self, wrapped, dart_error_out, error);
  if (object == NULL)
    return NULL;
...
static gboolean _gdart_bridge_context_invoke_static_on_dart (
  GdartBridgeContext *self,
  ...
  GError **error)
{
  ...
  Dart_Handle temp_result;
  gint *suppressed_args, suppressed_args_length;
  Dart_PersistentHandle function_persistent;
  const gchar *function_name = "Dart native function";

  function_persistent = Dart_NewPersistentHandle (function);
  if (!dart_callable_info.get_n_args (function_persistent,
                                      self,
                                      &garg_length,
                                      dart_error_out,
                                      error))
  ...
  Dart_NativeArguments arguments)
{
  ...
  if (result_object->object_info != NULL) {
    gchar *name, *namespace_;

    if (!result_object->object_info_klass.get_name (result_object->object_info,
        self,
        &name,
        &dart_error_out,
        &inner_error)) {
    ...
    if (!result_object->object_info_klass.get_namespace (result_object->object_info,
        self,
        &namespace_,
        &dart_error_out,
        &inner_error)) {
...
void gdart_bridge_context_call_dart_function_info (
  Dart_NativeArguments arguments)
{
  Dart_EnterScope();
  GdartBridgeContext *self;
  gintptr argument_count;
  Dart_Handle dart_error_out, function_info,
              dart_inner_error, args_list, final_result;
  GError *inner_error = NULL;
  ...
  if (argument_count < 2) {
    dart_error_out = gdart_bridge_context_create_error_handle (self,
                     "%s: takes 2 arguments, the function descriptor, and a list of arguments", G_STRFUNC);
    goto error;
  }
  function_info = Dart_GetNativeArgument (arguments, 0);
  if (G_UNLIKELY (Dart_IsError (function_info))) {
    dart_error_out = function_info;
    goto error_native;
  }
  args_list = Dart_GetNativeArgument (arguments, 1);
  if (G_UNLIKELY (Dart_IsError (args_list))) {
    dart_error_out = args_list;
    goto error_native;
  }
  ...
  if (!_gdart_bridge_context_invoke_static_on_dart (self,
      function_info,
      args_list,
      &final_result,
      &dart_inner_error,
  ...
  Dart_NativeArguments arguments)
{
  ...
  const RegisteredTypeInfoKlass *r_klass;
  gpointer struct_info;
  GIInfoType struct_info_type;
  gsize struct_info_size;
  gchar* namespace_;
  GType gtype;
  ...
  if (argument_count < 1) {
    dart_error_out = gdart_bridge_context_create_error_handle (self,
                     "%s: takes 1 arguments, the receiver (struct-info)",
                     G_STRFUNC);
    goto error;
  }
  receiver = Dart_GetNativeArgument (arguments, 0);
  ...
  if (is_of_type) {
    ...
    klass = gi_base_info_interface_info.cast_to_struct_union_info (result_base_info);
    r_klass = &gi_registered_type_info_registered_type_info;
    struct_info = g_base_info_ref (result_base_info);
  } else { //It should be implementing the struct-info interface.
    Dart_PersistentHandle handle;

    handle = Dart_NewPersistentHandle (receiver);
    klass = dart_interface_info.cast_to_struct_union_info (handle);
    r_klass = &dart_registered_type_info;
    struct_info = handle;
  }

  if (!klass->get_type (struct_info,
                        self,
                        &struct_info_type,
                        &dart_inner_error,
                        &inner_error)) {
    klass->free (struct_info);
    dart_error_out = temp_result;
    g_clear_error (&inner_error);
    goto error;
  }

  switch (struct_info_type) {
  case GI_INFO_TYPE_STRUCT:
  case GI_INFO_TYPE_UNION:
    if (!klass->get_size (struct_info,
                          self,
                          &struct_info_size,
                          &dart_inner_error,
                          &inner_error)) {
  ...
  if (struct_info_size == 0) {
    dart_error_out = gdart_bridge_context_create_error_handle (self,
                     "%s: was called on a struct info with no size set", G_STRFUNC);
    klass->free (struct_info);
    goto error;
  }
  gpointer raw_object = g_malloc0 (struct_info_size);
  GdartBridgeContextWrappedObject *object_wrapper = g_slice_new (GdartBridgeContextWrappedObject);
  object_wrapper->copy_func_ref = NULL; 
  //Not sure if this should use overriden
  //copy/free functions here.
  object_wrapper->free_func_ref = NULL;
  object_wrapper->copy_func = NULL;
  object_wrapper->free_func = NULL;
  object_wrapper->object = raw_object;
  object_wrapper->object_info = r_klass->copy (struct_info);
  object_wrapper->object_info_klass = *r_klass;
  Dart_Handle internal_container = gdart_bridge_context_wrap_pointer (self,
                                   object_wrapper,
  ...
  if (!klass->get_gtype(struct_info,
                        self,
			&gtype,
			&dart_inner_error,
			&inner_error)) {
  ...
  if (!klass->get_namespace(struct_info,
                            self,
			    &namespace_,
			    &dart_inner_error,
			    &inner_error)) {
  ...
  Dart_Handle wrapped_object = gdart_bridge_context_wrap_internal_pointer (
    self,
    internal_container,
    namespace_,
    gtype,
  ...
  Dart_SetReturnValue (arguments, wrapped_object);
  Dart_ExitScope();
  return;
error:
  dart_error_out = Dart_ThrowException (dart_error_out);
  ...
  Dart_NativeArguments arguments)
{
  ...
  GdartBridgeContext *self;
  gintptr argument_count;
  Dart_Handle dart_error_out, receiver, expected_type, temp_result, field_name_dart, expected_index_dart,
              dart_inner_error, dart_result_out;
  guchar *field_name_dart_untrimmed;
  gchar *field_name_dart_trimmed, *structure_name, *field_getter_name;
  gint64 expected_index, field_name_dart_length;
  
  bool is_of_type;
  GError *inner_error = NULL;
  gpointer field_info, write_buffer, field_type;
  GIInfoType struct_info_type;
  gint n_fields, raw_offset, offset;
  const StructUnionInfoKlass* struct_info_klass;
  const FieldInfoKlass* field_info_klass;
  const TypeInfoKlass* field_type_klass;
  gboolean is_end;
  GdartBridgeContextWrappedObject *result_wrapped_object;

  self = gdart_bridge_context_new_from_isolate (Dart_CurrentIsolate());
  argument_count = Dart_GetNativeArgumentCount (arguments);
  if (argument_count < 3) {
    dart_error_out = gdart_bridge_context_create_error_handle (self,
                     "%s: takes 1 arguments, the receiver (struct-info), the name of the field, and the expected index of the field",
                     G_STRFUNC);
    goto error;
  }
  receiver = Dart_GetNativeArgument (arguments, 0);
  ...
  result_wrapped_object = gdart_bridge_context_unwrap_object (self, receiver, &dart_inner_error, &inner_error);
  
  field_name_dart = Dart_GetNativeArgument(arguments, 1);
  if (G_UNLIKELY(Dart_IsError(field_name_dart))) {
    dart_error_out = field_name_dart;
    goto error_native;
  }
  temp_result = Dart_StringToUTF8(field_name_dart, &field_name_dart_untrimmed, &field_name_dart_length);
  if (G_UNLIKELY(Dart_IsError(temp_result))) {
    dart_error_out = temp_result;
    goto error_native;
  }
  field_name_dart_trimmed = g_newa(gchar, field_name_dart_length+1);
  memcpy(field_name_dart_trimmed, field_name_dart_untrimmed, field_name_dart_length);
  field_name_dart_trimmed[field_name_dart_length] = '\0';
  
  expected_index_dart = Dart_GetNativeArgument(arguments, 2);
  if (G_UNLIKELY(Dart_IsError(expected_index_dart))) {
    dart_error_out = temp_result;
    goto error_native;
  }
  temp_result = Dart_IntegerToInt64(expected_index_dart, &expected_index);
  if (G_UNLIKELY(Dart_IsError(temp_result))) {
    dart_error_out = temp_result;
    goto error_native;
  }
  ...
  if (G_UNLIKELY(!result_wrapped_object->object_info_klass.get_type(result_wrapped_object->object_info,
						    self,
						    &struct_info_type,
						    &dart_inner_error,
						    &inner_error))) {
  ...
  switch (struct_info_type) {
    ...
    default:
      dart_error_out = gdart_bridge_context_create_error_handle(self,
	                                                        "%s: can only get a raw field from struct or union",
								G_STRFUNC);
      goto error;
  }
  struct_info_klass = 
      result_wrapped_object->object_info_klass.cast_to_struct_union_info(
	   result_wrapped_object->object_info);
  if (G_UNLIKELY(!struct_info_klass->get_n_fields(result_wrapped_object->object_info,
				  self,
				  &n_fields,
				  &dart_inner_error,
				  &inner_error))) {
  ...
  if (n_fields <= expected_index) {
    g_warning("%s: The expected index for field '%s' was %ld, but the number of fields was only %d",
	      G_STRFUNC,
	      field_name_dart_trimmed,
	      expected_index,
	      n_fields);
  } else {
    gchar* field_name;
    if (G_UNLIKELY(!struct_info_klass->get_field(result_wrapped_object->object_info,
				 self,
				 expected_index,
				 &field_info,
				 &field_info_klass,
    ...
    if (G_UNLIKELY(!field_info_klass->get_name(field_info,
			       self,
			       &field_name,
			       &dart_inner_error,
			       &inner_error))) {
      field_info_klass->free(field_info);
      dart_error_out = dart_inner_error;
      g_clear_error(&inner_error);
      goto error;
    }
    if (g_strcmp0(field_name, field_name_dart_trimmed) != 0) {
      field_info_klass->free(field_info);
      field_info = NULL;
      field_info_klass = NULL;
    }
    g_free(field_name);
  }
  if (field_info == NULL) {
    for (int i = 0; i < n_fields && field_info == NULL; i++) {
      gchar* field_name;
      if (G_UNLIKELY(!struct_info_klass->get_field(result_wrapped_object->object_info,
	self,
	i,
    ...
    if (field_info == NULL) {
      dart_error_out = gdart_bridge_context_create_error_handle(self,
	                                                        "%s: can't find the specified field",
								G_STRFUNC);
      goto error;
    }
  }
  if (G_UNLIKELY(!field_info_klass->get_offset(field_info,
                                               self,
					       &raw_offset,
					       &dart_inner_error,
					       &inner_error))) {
  ...
  if (G_UNLIKELY(!struct_info_klass->get_name(result_wrapped_object->object_info,
      self,
      &structure_name,
      &dart_inner_error,
      &inner_error))) {
  ...
  field_getter_name = g_strdup_printf("%s.get %s", structure_name, field_name_dart_trimmed);
  g_free(structure_name);
  
  write_buffer = (gpointer) (((gchar*) result_wrapped_object->object) + raw_offset);
  /*
  g_warning("%s: Field is %s.%s\n    Raw offset is %d\n    Object is at %p\n    Offset is %d\n    Buffer to write is at %p\n",
            G_STRFUNC,
	    structure_name,
	    field_name_dart_trimmed,
	    raw_offset,
	    result_wrapped_object->object,
	    offset,
	    write_buffer);
  */
  
  
  if (G_UNLIKELY(!field_info_klass->get_type(field_info,
                                             self,
					     &field_type,
					     &field_type_klass,
					     &dart_inner_error,
					     &inner_error))) {
    field_info_klass->free(field_info);
    g_free(field_getter_name);
    dart_error_out = dart_inner_error,
    g_clear_error(&inner_error);
    goto error;
  }
  if (G_UNLIKELY(!gdart_array_load_element (self,
                                            field_getter_name,
					    field_type,
					    field_type_klass,
					    FALSE,
					    ...
					    &inner_error))) {
    field_info_klass->free(field_info);
    field_type_klass->free(field_type);
    g_free(field_getter_name);
    dart_error_out = dart_inner_error,
    g_clear_error(&inner_error);
    goto error;
  ...
  field_info_klass->free(field_info);
  field_type_klass->free(field_type);
  if (Dart_IsNull(dart_result_out)) {
    g_warning("About to report null.");
  }
  Dart_SetReturnValue(arguments, dart_result_out);
